...
export default function CreateAirdrop() {
  ...
  const useApiForWrittingFile = async (
  ...
  const downloadJson = async (airdropAmountList: airdropListData[]) => {
    let merkletree = JSON.stringify(parseBalanceMap(airdropAmountList));
  ...
  const generateAirdropList = async () => {
    ...
    if (snapshotTokenAddress2Value !== "") {
      ...
      responseJson = (await response.json()) as numberResponse;
